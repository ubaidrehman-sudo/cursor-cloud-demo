...
describe('CreateTaskDto', () => {
  ...
  it('requires task titles to be at least 3 characters long', async () => {
    const errors = await validateDto({ title: 'ab', priority: 'medium' });
  ...
  it('allows task titles within the 3 to 100 character range', async () => {
    await expect(validateDto({ title: 'abc', priority: 'medium' })).resolves.toHaveLength(
      0,
    );
    await expect(
      validateDto({ title: 'a'.repeat(100), priority: 'medium' }),
    ).resolves.toHaveLength(0);
  });

  it('requires task titles to be at most 100 characters long', async () => {
    const errors = await validateDto({ title: 'a'.repeat(101), priority: 'medium' });
  ...
  it('requires task priority to be provided', async () => {
    const errors = await validateDto({ title: 'abc' });
